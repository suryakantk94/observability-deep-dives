...
- [**Pre-aggregating metrics at ingest with rule-based shaping**](rule-based-metric-shaping.md) — an end-to-end walk-through of a cardinality-control pattern: divert high-cardinality metrics to a separate storage path, pre-aggregate via recording rules, and route queries back to the right table. ASCII architecture diagrams included.
